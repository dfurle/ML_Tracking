...
G4VPhysicalVolume* B5DetectorConstruction::Construct()
{
  ...
  auto worldSolid 
    = new G4Box("worldBox",20.*m,3.*m,15.*m);
  auto worldLogical
  ...
  auto magneticSolid 
    = new G4Tubs("magneticTubs",0.,2.*m,2.*m,0.,360.*deg);
  ...
  // second arm
  auto secondArmSolid 
    = new G4Box("secondArmBox",5.*m,2.*m,3.5*m);
  auto secondArmLogical
    = new G4LogicalVolume(secondArmSolid,air,"secondArmLogical");
  auto x = -6.*m * std::sin(fArmAngle);
  auto z = 6.*m * std::cos(fArmAngle);
  fSecondArmPhys
  ...
  for (auto i=0;i<kNofChambers;i++) {
    ...
    new G4PVPlacement(rotate,G4ThreeVector(0.,z1,0.),chamber1Logical,
                      "chamber1Physical",fMagneticLogical,
                      false,i,checkOverlaps);
  }

  // drift chamber in first arm
  auto chamberFSolid = new G4Box("chamberFBox",1.*m,30.*cm,1.*cm);
  auto chamberFLogical = new G4LogicalVolume(chamberFSolid, argonGas,"chamberFLogical");
  auto rotate = new G4RotationMatrix();
  new G4PVPlacement(rotate,G4ThreeVector(0.,0.,2.9*m),chamberFLogical,
                    "chamberFPhysical",firstArmLogical,
                    false,0,checkOverlaps);
  ...
  auto wirePlaneFSolid = new G4Box("wirePlaneFBox",1.*m,30.*cm,0.1*mm);
  fWirePlaneFLogical = new G4LogicalVolume(wirePlaneFSolid,argonGas,"wirePlaneFLogical");
  new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),fWirePlaneFLogical,
                    "wirePlaneFPhysical",chamberFLogical,
                    false,0,checkOverlaps);
  ...
  chamberFLogical->SetVisAttributes(visAttributes);
  // secondArmLogical->SetVisAttributes(visAttributes);
  ...
  fWirePlaneFLogical->SetVisAttributes(visAttributes);
...
void B5DetectorConstruction::ConstructSDandField()
{
  ...
  auto chamberF = new B5DriftChamberSD(SDname="/chamberF");
  sdManager->AddNewDetector(chamberF);
  fWirePlaneFLogical->SetSensitiveDetector(chamberF);
